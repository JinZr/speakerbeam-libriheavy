...
class LibriMixInformed(Dataset):
    def __init__(
        ...
        if segment_aux is not None:
            ...
            self.data_aux = {
                m: {
                    u: [
                        (path, length)
                        for path, length in self.data_aux[m][u]
        ...
        self.data_aux_list = [(m, u) for m in self.data_aux for u in self.data_aux[m]]
    ...
    def __getitem__(self, idx):
        ...
        row = self.base_dataset.df[
            self.base_dataset.df["mixture_ID"] == mix_id
        ].squeeze()
